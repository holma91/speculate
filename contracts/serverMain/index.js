...
const { fuji, rinkeby, binance } = require('../scripts/addresses');
...
fs.readFile('./serverMain/makerAsks.json', 'utf8', (_, storedMakerAsks) => {
  makerAsks = JSON.parse(storedMakerAsks);
});

fs.readFile('./serverMain/makerBids.json', 'utf8', (_, storedMakerBids) => {
...
speculateExchange.on(
  ...
  async (taker, maker, tokenId, currency, collection, amount, price) => {
    ...
    if (makerAsks[collection] && makerAsks[collection][tokenId]) {
...
speculateExchange.on(
  ...
  async (taker, maker, tokenId, currency, collection, amount, price) => {
    ...
    collection = collection.toLowerCase();
    delete makerAsks[collection][tokenId];
    if (makerBids[collection] && makerBids[collection][tokenId]) {
      delete makerBids[collection][tokenId];
    }
    fs.writeFile(
      './serverMain/makerAsks.json',
